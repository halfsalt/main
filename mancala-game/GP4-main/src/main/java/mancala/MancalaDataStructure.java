...
public class MancalaDataStructure implements Serializable {
    ...
    private final static int GAME_STORE = 2;
    private int startStone = 4;  //not final because we might want a different size board in the future

    // uses a list to represent the pits and stores on each board
    // each countable object can be with a certain number of stones or a store 
    ...
    public MancalaDataStructure() {
     ...
     /*
     * This method will return the countable array
     */
    public List<Countable> getCountables() {
        return data;
    }
    ...
    public void clearPits() {
        for (int i = 1; i <= 12; i++) {
            data.get(pitPos(i)).removeStones(); // Set each pit to zero
        }
    }
    ...
    public void setUpPits() {
        for (int i = 0; i < PLAYER_ONE; i++) {
            data.get(i).addStones(startStone);
        }
    ...
    public void setUpAyoBoard() {
        // Assume pits and stores are indexed from 0 to 13, where 0 and 7 are the stores
        for (int i = 1; i <= 12; i++) { // Pits are indexed from 1 to 12
            if (i != 1 && i != 7) { // Exclude pit 1 and 7 for Ayo setup
                addStones(i, 4); // Add 4 stones to each pit except 1 and 7
            } else {
                addStones(i, 0); // Ensure pits 1 and 7 have 0 stones
            }
        }
    }
